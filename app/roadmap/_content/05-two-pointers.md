...
|---|---------|--------|-----------|------|------|-----------------|
| 1 | [Sum of Two Values](https://cses.fi/problemset/task/1640) | CSES | Easy | UG ⭐ | baseline | Converging two-sum on sorted array — you solved this in module 2 with a dict (complement lookup); now sort + two pointers, then map back to original indices via `(value, index)` tuples |
| 2 | [Squares of a Sorted Array](https://leetcode.com/problems/squares-of-a-sorted-array/) | LC 977 | Easy | new | baseline | New sub-pattern: magnitude-comparison converging. Sorted input has negatives, so the largest square sits at one end — compare `abs(a[left])` vs `abs(a[right])`, write the winner to `out[write]`, decrement `write`. Element-emitting loop uses `while left <= right`, not `<` |
| 3 | [Valid Palindrome](https://leetcode.com/problems/valid-palindrome/) | LC 125 | Easy | NC150 | extension | Converging on a string from both ends — character-skipping edge case (filter non-alphanumeric inline) |
| 4 | [Sum of Three Values](https://cses.fi/problemset/task/1641) | CSES | Easy | UG ⭐ | extension | Reduce by fixing one — fix outer index `i`, run converging two-pointer on the suffix. No dedup needed (distinct values) — isolates the skeleton from problem 5's wrinkle |
| 5 | [3Sum](https://leetcode.com/problems/3sum/) | LC 15 | Medium | NC150 | extension | Same skeleton as problem 4 plus deduplication — skip duplicate outer `a[i]`; advance `left`/`right` past their duplicates after recording a hit |
| 6 | [Container With Most Water](https://leetcode.com/problems/container-with-most-water/) | LC 11 | Medium | NC150 | extension | Different objective (max area, not match a target). Now solve the Step 1 cold attempt with the monotonic invariant from Step 2 |
| 7 | [Trapping Rain Water](https://leetcode.com/problems/trapping-rain-water/) | LC 42 | Hard | NC150 | **checkpoint** | Converging while maintaining `left_max` and `right_max` in one pass; combines the monotonic invariant with the [two-scan span](00-patterns.md#two-scan-span) from module 4 |

**Checkpoint:** LC 42 without hints. The two-pointer approach maintains running `left_max` and `right_max`. At each step, whichever side has the smaller max determines the water trapped on that side (because the other side guarantees a wall at least as tall). This combines the monotonic invariant (the smaller side decides) with the two-scan span idea (prefix max from the left, suffix max from the right) — but compressed into a single pass rather than two. That compression is the leap.

---

## Common mistakes

- **Forgetting to sort.** Converging two pointers require a sorted array (or another monotonic relationship). On an unsorted array, moving a pointer doesn't change the objective in a predictable direction. If sorting changes the output (e.g., CSES 1640 returns original indices), sort `(value, original_index)` pairs and map back at the end.
- **`while left <= right` vs `left < right`.** Two cases, different rules. Pair-finding loops (2-sum, palindrome, container) use `left < right` — once they meet there's no pair left, and the middle element doesn't pair with itself. Element-emitting loops (LC 977 squares, write-from-back, in-place merge) use `left <= right` — the middle element still needs to be placed into the output. Pick based on whether the body needs both indices distinct or just non-crossing.
- **Filling the magnitude-comparison output from the front.** Writing the larger square to `out[0]`, `out[1]`, … gives a *descending* sequence and forces a reverse at the end. Instead, write to `out[n-1]`, `out[n-2]`, … — the result is already sorted ascending and you save the O(n) reversal pass.
- **3Sum dedup placement.** Skip duplicate outer `a[i]` *after* the first iteration (`if i > 0 and a[i] == a[i-1]`). Skip duplicate `left` and `right` *after* recording a valid triple, before advancing the pointers. Skipping the outer at iteration 0 misses valid triples; skipping inner pointers before recording loses the triple entirely.
