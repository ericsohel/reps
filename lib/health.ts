...
import { db } from "./db";
import { problems, attempts, srsState, patternState, stars, starSrs, companies, applications, counters } from "./schema";
...
export async function ensureSchemaHealthy() {
  ...
  try {
    await Promise.all([
      ...
      db.select({ n: sql<number>`count(*)` }).from(applications).limit(1),
